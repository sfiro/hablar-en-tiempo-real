...
terminal. Pedido explícito desde v8: la conversación de voz vive en el navegador, sin
lag; a este proceso de Python solo debe llegarle el texto ya transcrito, para el
análisis de sentimiento y el envío a la Pico.

Novedad de v9: con --tracking, este proceso también rastrea el rostro por cámara
(`face_tracker.py`, retomado de v7) en un hilo de fondo, y manda la posición real —no
un valor fijo— junto con cada comando a la Pico. --sentiment y --tracking son
independientes entre sí: se puede rastrear sin sentimiento, tener sentimiento sin
rastreo (como en v8), o las dos juntas (el caso pensado para esta versión).

Novedad, añadida sobre v9: modo dormido por inactividad. Si pasan
...
Este proceso hace hasta cinco cosas:
  ...
  4. (Con --tracking, nuevo en v9) Rastrea el rostro por cámara en un hilo de fondo y
     manda su posición real por serial mientras haya cambios significativos —
     independiente de si --sentiment está activo.
...
Uso:
    ...
    python webrtc_server.py --sentiment --tracking   # el caso pensado para v9
...
# Mismo mapeo que v9/realtime_voice.py — ver ese fichero para el razonamiento
...
def _hilo_rastreo(cap, detener: threading.Event):
    ...
    Recibe un `cv2.VideoCapture` ya abierto, en vez de abrirlo aquí — hallazgo
    real, no anticipado: abrir la cámara desde este hilo falla en macOS con
    "OpenCV: can not spin main run loop from other thread", porque
    AVFoundation necesita negociar el permiso de cámara desde el run loop
    principal. Se resuelve abriendo `cv2.VideoCapture` en `main()` (hilo
    principal) y pasando el objeto ya abierto aquí — este hilo solo lee
    frames de un capturador que ya negoció el permiso.
    """
    import cv2
...
def main():
    ...
    # --- Opciones de v9: rastreo facial real ------------------------------------
    parser.add_argument("--tracking", action="store_true",
                        help="Rastrea el rostro por cámara y manda su posición real a la "
                             "Pico. Independiente de --sentiment: se puede usar solo, o "
                             "junto con --sentiment (el caso pensado para esta versión). "
                             "Necesita 'opencv-python' instalado.")
    parser.add_argument("--camera-index", type=int, default=0,
                        help="Índice de cámara para cv2.VideoCapture (por defecto: 0). "
                             "Solo tiene efecto con --tracking.")
    ...
    if (args.sentiment or args.tracking) and args.usar_pico:
        ...
        if puerto_pico:
            ...
            print("⚠️  No se detectó ninguna Pico. La conversación sigue "
                  "igual, sin mover los servos.")

    if args.tracking:
        # cv2.VideoCapture se abre AQUÍ, en el hilo principal — no dentro de
        # _hilo_rastreo(). En macOS, abrirla desde un hilo de fondo falla
        # ("can not spin main run loop from other thread"): AVFoundation
        # necesita negociar el permiso de cámara desde el run loop principal.
    ...
    if args.tracking:
        print(f"    Rastreo facial: ACTIVO (cámara {args.camera_index})")
    if PICO is not None:
