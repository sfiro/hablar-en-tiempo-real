...
Desde v9 esto ya no es una posibilidad futura: `webrtc_server.py` importa
`FaceTracker` directamente y la usa en un hilo de fondo mientras la conversación de
voz corre en el hilo principal (ver PLAN-v9.md).

Copia idéntica de `../v7/face_tracker.py` (que a su vez viene de v6, v5, v4, v3, y
    ...
    try:
        while True:
            ...
            if mostrar_ventana:
                ...
                cv2.imshow("Rastreo Facial (v9, prueba standalone)", frame)
...
def main():
    parser = argparse.ArgumentParser(
        description="Prueba standalone del rastreo facial (ver PLAN-v9.md).")
