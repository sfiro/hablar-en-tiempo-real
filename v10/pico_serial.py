...
Adaptado de `../v9/pico_serial.py` (que a su vez viene de v8, v7, v6, v5, v4,
v3) para el cambio de cliente de v10: hasta v9 hablaba con la Pico un Mac, desde
v10 es una Raspberry Pi 5. El único cambio real es la detección de puerto —
`encontrar_puerto_mac()` buscaba `/dev/cu.usbmodem*`/`/dev/tty.usbmodem*`
(nombres de macOS); en Linux/Raspberry Pi OS la Pico enumera como dispositivo
CDC-ACM, típicamente `/dev/ttyACM0`. Resto del fichero sin cambios de lógica.
Compatible con `main.py` en esta misma carpeta, que usa el campo EMOCION para
decidir la expresión facial (ver la cabecera de `main.py`). Cada versión de
este proyecto es autónoma y no importa código de otra carpeta de versión — ver
CLAUDE.md.
