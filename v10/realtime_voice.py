...
Este fichero, a diferencia de v10/webrtc_server.py, sigue sin rastreo facial —
decisión explícita de alcance heredada de v9: la cámara solo se añadió a la
versión recomendada (navegador), no a esta alternativa de terminal. Sigue
mandando un LR,UD fijo (90,90) junto con cada EMOCION.

Copia de v9/realtime_voice.py, adaptada para el cambio de cliente de v10 (Mac
→ Raspberry Pi 5): el único cambio real es el nombre de la función que
localiza el puerto serial de la Pico (`encontrar_puerto_pico`, en
`pico_serial.py` — busca `/dev/ttyACM*`, el path de Linux, en vez de los
nombres de macOS). El micro/parlante USB los usa `sounddevice` (PortAudio) por
el dispositivo por defecto del sistema — en Raspberry Pi OS hay que dejarlos
como entrada/salida por defecto antes de correr esto (ver README-v10.md).
    ...
    async with websockets.connect(
        ...
        async def analyze_and_print(role: str, text: str):
            ...
            if pico is not None:
                ...
                # v10/webrtc_server.py), así que LR/UD van fijos al centro —
