# v10 — Firmware MicroPython para la Raspberry Pi Pico.
#
...
#
# Sin cambios de lógica respecto a v8/v9 — este firmware ya aceptaba LR/UD
# reales por serial (para las 7 expresiones que no fijan la mirada) sin
# distinguir si venían de un rastreo real o de un valor fijo, ni de qué
# dispositivo salían. La novedad de v10 está del lado del cliente: hasta v9
# hablaba con esta Pico un Mac; desde v10 es una Raspberry Pi 5 (con cámara
# CSI para el rastreo y micrófono/parlante USB para la voz) — mismo protocolo
# serial de siempre, así que este fichero no necesitó ningún cambio.
#
# Cambio de fondo respecto a v6/v7, ya presente desde v8: hasta ahora, la
# expresión cambiaba sola cada 5 segundos, cicladas en un orden fijo, sin
# depender de nada externo. Desde v8 eso se reemplaza por un modelo dirigido
# por eventos: cada vez que llega "LR,UD,EMOCION\n" con un campo EMOCION
# válido, la Pico adopta esa expresión y la mantiene durante
# INTERVALO_EXPRESION_MS (5000ms, el mismo valor de antes, ahora usado como
# duración del "pulso" en vez de periodo del ciclo) — pasado ese tiempo sin una
# emoción nueva, vuelve sola a NEUTRAL, que es el estado base. Pensado para que
# el cliente (v10/webrtc_server.py, en la Raspberry Pi 5) envíe la emoción
# detectada en cada frase de la conversación (ver sentiment_analyzer.py y el
# mapeo a las 10 emociones de la Pico en README-v10.md) — la cara reacciona a
# la conversación, no a un temporizador ciego. Los offsets de párpados/cuello
# por emoción son los mismos de v6/v7 (OFFSETS_EMOCIONES, con FELIZ y SOSPECHA
# recalculados — ver el comentario junto a OFFSETS_EMOCIONES), aplicados sobre
# una posición de reposo fija de los párpados, no completamente abierta (esta
# base no sincroniza los párpados con la mirada todavía, a diferencia del
# original — ver PARPADOS_REPOSO).
#
...
# construidas desde v6, sin cambios aquí. Para las otras 7, desde v9 esa
# mirada sí es la del rostro rastreado de verdad (ahora por la cámara CSI de
# la Pi 5 en vez de la cámara del Mac).
#
...
# descrito arriba. El cliente (pico_serial.py, copia propia de v10, adaptada
# para hablar desde Linux/Raspberry Pi OS en vez de macOS) no necesita ningún
# cambio de protocolo — es el mismo de siempre.
...
# ==========================================
print("v10: iniciando (rastreo de ojos + cuello + parpadeo + expresión dirigida por EMOCION)...")
...
def actualizar_objetivo_expresion():
    ...
    de los párpados (esta base no sincroniza párpados con la mirada todavía) y, salvo
