...
Novedad de v8: con --sentiment activo, si hay una Pico conectada (autodetectada,
igual que face_tracker.py en versiones anteriores), cada emoción detectada por
encima de --confidence-threshold se traduce al vocabulario de 10 emociones de la
Pico (EMOTION_TO_PICO, más abajo) y se envía por serial. El firmware (main.py)
mantiene esa expresión 5 segundos y, si no llega una nueva, vuelve solo a
NEUTRAL — la cara reacciona a la conversación, no a un temporizador ciego. v8
todavía no incluye rastreo facial (eso es v9): se manda un LR,UD fijo (90,90).
Usa --no-pico para desactivar el envío aunque haya una Pico conectada.
    ...
    async with websockets.connect(
        ...
        async def analyze_and_print(role: str, text: str):
            ...
            if pico is not None:
                ...
                # 90,90: v8 todavía no rastrea el rostro (eso es v9), así que
                # LR/UD van fijos al centro — solo importa el campo EMOCION.
