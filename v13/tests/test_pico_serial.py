...
en v12 tras el bug real de buffer USB desbordado — ver la cabecera de
pico_serial.py y ../v12/MODIFICACIONES-LOCALES.md).
...
def test_comando_llega_al_serial_falso():
    ...
    link = PicoLink(port="/dev/ttyACM_FAKE", serial_factory=fabrica)
    link.start()
    try:
...
def test_latido_reenvia_el_ultimo_comando():
    ...
    link = PicoLink(port="/dev/ttyACM_FAKE", serial_factory=fabrica, heartbeat_s=0.1)
    link.start()
    try:
        for _ in range(20):
            if link.esta_conectado():
...
# --- _drenar_entrada() — bug real del buffer USB desbordado (v12, 23/08/2026) ----
