# v13 — Firmware MicroPython para la Raspberry Pi Pico.
#
# Copia idéntica de ../v12/main.py (== ../v9/main.py), SIN NINGÚN CAMBIO DE
# LÓGICA. v13 junta voz en tiempo real (retomada de v11) con rastreo facial
# real (retomado de v12) en el mismo proceso del lado de la Pi 5 — el
# firmware no necesita saberlo: sigue aceptando "LR,UD\n" y
# "LR,UD,EMOCION\n" por USB serial exactamente igual que desde v8/v9,
# sin que le importe qué máquina ni qué proceso concreto se lo manda.
#
# v13 no envía sentimiento todavía (a propósito, ver ../v13/README-v13.md):
# el cliente nunca manda un EMOCION, así que emocion_actual se queda en su
# valor inicial, "NEUTRAL", durante toda la sesión — el parpadeo periódico
# sigue activo (no se desactiva salvo con DORMIDO, que aquí nunca se manda)
# y los párpados quedan en su posición de reposo normal. El rastreo sí
# mueve LR/UD de verdad, con el cuello seguido detrás.
#
...
#
# Modelo dirigido por eventos (sin cambios desde v8): cada vez que llega
# "LR,UD,EMOCION\n" con un campo EMOCION válido, la Pico adopta esa expresión y
# la mantiene durante INTERVALO_EXPRESION_MS (5000ms); pasado ese tiempo sin una
# emoción nueva, vuelve sola a NEUTRAL. En v9 eso lo disparaba cada frase de la
# conversación (vía sentiment_analyzer.py); en v12 lo disparaba un temporizador
# de ciclo fijo; en v13 no se dispara nunca (sin sentimiento todavía) — el
# firmware no distingue una cosa de la otra, solo reacciona al campo EMOCION
# que le llega, o a su ausencia.
#
# DUDA, PENSATIVO y NERVIOSO siguen ignorando LR/UD (real o no) y fijando o
# moviendo la mirada por su cuenta mientras están activas — así fueron
# construidas desde v6, sin cambios aquí. En v13 nunca se activan (no hay
# EMOCION que las dispare), así que las 10 expresiones siguen todas la
# mirada real del rastreo por defecto (NEUTRAL no fija nada).
#
# Por qué no hay PCA9685: en v5, la primera versión de este firmware sí lo usaba
# (sigue disponible en ../v5/main_pca9685.py, como referencia histórica) y daba
# tres problemas en hardware real: temblor aleatorio al encender, un temblor
# periódico disparado por el parpadeo, y el eje PAN sin moverse en absoluto. Al
# quitar el PCA9685 y generar el PWM directo desde la Pico, los tres problemas
# desaparecieron a la vez. Cronología completa en ../v5/README-v5.md, sección
...
# (pico_serial.py, copia propia de v13 adaptada de v10/v12 para Linux) no
# necesita ningún cambio — el protocolo es el mismo de siempre.
...
# antes de volver sola a NEUTRAL si no llega una emoción nueva. Sin efecto
# práctico en v13 (nunca llega un EMOCION), se conserva por compatibilidad con
# el mismo firmware que usan v9/v12.
...
# ==========================================
print("v13: iniciando (rastreo de ojos + cuello + parpadeo + voz en tiempo real, sin sentimiento todavía)...")
...
def actualizar_objetivo_expresion():
    ...
    de los párpados (v9/v12/v13 no sincronizan párpados con la mirada todavía)
    y, salvo en TRISTE (ver abajo), los suma al TILT ya calculado por
...
def actualizar_objetivo_mirada_expresion():
    ...
    mandando. En v13 esta función nunca actúa en la práctica (emocion_actual
    se queda siempre en NEUTRAL), pero se conserva sin cambios porque es el
    mismo firmware que v9/v12."""
...
def parpadear():
    ...
    posición de reposo de los párpados ya no es la neutral. En v13,
    emocion_actual es siempre NEUTRAL, así que siempre reabre a la posición de
    reposo — el parpadeo periódico sigue funcionando con normalidad durante
...
try:
    while True:
        ...
        # por el offset) — mismo criterio que ojosMecanicos/main.py. En v13
        # DORMIDO nunca llega, así que el parpadeo nunca se desactiva.
        ...
        # nueva: vuelve sola a NEUTRAL, el estado base. Sin efecto práctico en
        # v13 (emocion_actual nunca deja de ser NEUTRAL).
