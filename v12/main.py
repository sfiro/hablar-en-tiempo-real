# v12 — Firmware MicroPython para la Raspberry Pi Pico.
#
# Copia idéntica de ../v9/main.py, SIN NINGÚN CAMBIO DE LÓGICA. v12 no toca el
# firmware en absoluto: el cambio de esta versión está del lado del cliente
# (../v12/rastreo_expresiones.py, nuevo), que ahora corre en una Raspberry Pi 5
# en vez de un Mac. Este fichero ya aceptaba "LR,UD,EMOCION\n" por USB serial
# desde v8/v9 sin que le importara qué máquina se lo mandaba — v9 se lo mandaba
# un Mac (con face_tracker.py + webrtc_server.py); v12 se lo manda una Pi 5 (con
# face_tracker.py + rastreo_expresiones.py, sin voz todavía). Ver la cabecera de
# ../v12/rastreo_expresiones.py para el detalle de qué cambia y qué no.
#
...
# conversación (vía sentiment_analyzer.py); en v12 lo dispara un temporizador de
# ciclo fijo del lado de la Pi 5 (ver rastreo_expresiones.py) — el firmware no
# distingue una cosa de la otra, solo reacciona al campo EMOCION que le llega.
#
...
# construidas desde v6, sin cambios aquí. Para las otras 7, la mirada es la del
# rostro rastreado de verdad por la cámara de la Pi 5.
#
...
# (pico_serial.py, copia propia de v12 adaptada de v10 para Linux) no necesita
...
# el periodo del ciclo fijo de rastreo_expresiones.py (5000ms por defecto),
# reutilizado con otro propósito.
...
# ==========================================
print("v12: iniciando (rastreo de ojos + cuello + parpadeo + expresión dirigida por EMOCION)...")
...
def actualizar_objetivo_expresion():
    ...
    de los párpados (v9/v12 no sincronizan párpados con la mirada todavía) y,
    salvo en TRISTE (ver abajo), los suma al TILT ya calculado por
