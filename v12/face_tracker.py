...
—EMA, zona muerta, mapeo a grados— **no cambió ni una línea**. Cada versión de este
...
Lo único que cambia respecto a v9 es la cámara: a diferencia de v10 (Raspberry Pi 5
con cámara **CSI**, que necesitó `picamera2`/`libcamera` porque `cv2.VideoCapture` no
habla ese bus), la cámara de v12 es una **webcam USB** conectada a la propia Pi 5,
enumerada por el driver V4L2 genérico como `/dev/video1` — exactamente el mismo tipo
de dispositivo que `cv2.VideoCapture` ya sabía abrir desde v3 (Mac). Por eso v12 no
necesita ningún wrapper nuevo como `abrir_camara_csi()`: solo cambia el **índice**
por defecto que se le pasa a `cv2.VideoCapture` (1, no 0 — ver `--camera-index` más
abajo), y de dónde viene la Pico (`pico_serial.encontrar_puerto_pico()`, adaptado
para Linux desde v10, en vez de `encontrar_puerto_mac()`).
...
Uso como script independiente (con ventana de depuración, para probar la cámara):
    ...
    python face_tracker.py --camera-index 0   # si tu cámara no es /dev/video1
"""
...
def _run_standalone(mostrar_ventana: bool, usar_pico: bool, camera_index: int):
    ...
    Esto es solo para validar la cámara y el detector solos, sin el ciclo de
    expresiones de rastreo_expresiones.py.
    """
...
def main():
    parser = argparse.ArgumentParser(
        description="Prueba standalone del rastreo facial (ver PLAN-v12.md).")
