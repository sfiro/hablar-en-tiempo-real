...
cuando se integre con la conversación de voz (ver PLAN-v4.md).

Copia idéntica de la versión en `../v3/face_tracker.py` (que a su vez viene de
`ojosMecanicos/rastreoCara_Mac.py`). Cada versión de este proyecto es autónoma y no
importa código de otra carpeta de versión, así que este fichero se duplica en vez de
referenciarse — ver CLAUDE.md, sección "Cada versión es independiente".
    ...
    try:
        while True:
            ...
            if mostrar_ventana:
                ...
                cv2.imshow("Rastreo Facial (v4, prueba standalone)", frame)
...
def main():
    parser = argparse.ArgumentParser(
        description="Prueba standalone del rastreo facial (ver PLAN-v4.md).")
